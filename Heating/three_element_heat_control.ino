...
void zc_interrupt(){
  ...
  // oven >= inj_det + 8.33
  if(float diff = dim_oven - (dim_inj_det + 8.33) >=0){
  ...
  else if(float diff = dim_oven - dim_inj_det > 0){
  ...
  // inj_det >= oven +8.33
  else if(float diff = dim_inj_det - (dim_oven + 8.33) >= 0){
  ...
  else if(float diff = dim_inj_det - dim_oven  >= 0){
    delayMicroseconds(dim_oven);
    digitalWrite(triac_oven,HIGH);
    delayMicroseconds(8.33-diff);
    digitalWrite(triac_inj_det,HIGH);
